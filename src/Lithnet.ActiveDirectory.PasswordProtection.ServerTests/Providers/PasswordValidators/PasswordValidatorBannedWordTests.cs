...
{
    [TestFixture()]
    public class PasswordValidatorBannedWordTests
    {
        private IPasswordValidator CreateValidator()
        {
            Mock<IStore> store = new Mock<IStore>();
            store.Setup(t => t.IsInStore("password", StoreType.Word)).Returns(true);

            Mock<IStoreProvider> storeProvider = new Mock<IStoreProvider>();
            storeProvider.SetupGet(t => t.Store).Returns(store.Object);

            return new PasswordValidatorBannedWord(Global.LogFactory.CreateLogger("Test"), storeProvider.Object);
        }
        ...
        public void ApprovedOnPolicyDisabled()
        {
            ...
            {
                NormalizedBannedWordChecking = false
            };

            PasswordValidationRequest request = new PasswordValidationRequest { Password = "p@ssword!" };
        ...
        public void FailsOnBadPassword()
        {
            ...
            Policy policy = new Policy()
            {
                NormalizedBannedWordChecking = true
            };

            PasswordValidationRequest request = new PasswordValidationRequest { Password = "p@ssword!" };

            Assert.AreEqual(PasswordTestResult.BannedNormalizedWord, validator.ValidatePassword(request, policy).Result);
        }
