...
namespace PasswordFilterPS
{
    ...
    public class ImportBreachedPasswordHashes : PSCmdlet
    {
        ...
        protected override void ProcessRecord()
        {
            var task = Task.Run(() =>
            {
                try
                {
                    ...
                    if (isSorted)
                    {
                        StoreInterface.Store.ImportHexHashesFromSortedFile(Global.Store, StoreInterface.StoreType.Password, this.Filename, this.token.Token, this.progress);
                    }
                    else
                    {
                        StoreInterface.Store.ImportHexHashesFromFile(Global.Store, StoreInterface.StoreType.Password, this.Filename, this.token.Token, this.BatchSize, this.progress);
                    }
                }
            ...
            var p = new ProgressRecord(1, "Importing hashes", "Starting...");
